...
class ESCPOSPrinter:
    ...
    # ESC 7 parameter limits (used for clamping config values)
    HEAT_TIME_MIN = 5
    HEAT_TIME_MAX = 50
    MAX_DOTS_MIN = 0
    MAX_DOTS_MAX = 30

    # Fixed heat interval — not user-configurable
    HEAT_INTERVAL = 10 # DONT NOT GO ABOVE 30, I REPEAT DO NOT GO ABOVE 30 YOU WILL DAMAGE YOUR PRINTER!!!

    def __init__(self, retry_attempts: int = 3, line_spacing: int = 30,
                 heat_time: int = 30, max_dots: int = 5):
        """
        Initialize ESC/POS printer.
        
        Args:
            retry_attempts: Number of retry attempts for connection
            line_spacing: Dots per newline (0–255). Default 30 ≈ 3.8 mm.
            heat_time: ESC 7 heating time in 10 µs units (5–50). Default 30.
            max_dots: ESC 7 max heating dots (0–30). Default 5.
        """
        ...
        self.line_spacing = max(0, min(255, line_spacing))  # clamp to valid ESC/POS range
        self.heat_time = max(self.HEAT_TIME_MIN, min(self.HEAT_TIME_MAX, heat_time))
        self.max_dots = max(self.MAX_DOTS_MIN, min(self.MAX_DOTS_MAX, max_dots))
        self.heat_interval = self.HEAT_INTERVAL
    ...
    def print_image(self, image_path: str, auto_reconnect: bool = True) -> bool:
        ...
        for attempt in range(self.retry_attempts if auto_reconnect else 1):
            ...
            try:
                ...
                printer._raw(b'\x1b\x40')  # ESC @ — initialize/reset
                
                # Set line spacing (ESC 3 n) — controls how far each \n advances
                printer._raw(bytes([0x1B, 0x33, self.line_spacing]))
                
                # Set print heat parameters (ESC 7 n1 n2 n3)
    ...
    def _apply_heat_settings(self, printer) -> None:
        """
        Send ESC 7 command to configure print energy.

        ESC 7 (0x1B 0x37) parameters:
            n1 = max printing dots  (self.max_dots,     0–30)
            n2 = heating time       (self.heat_time,    5–50)   ×10 µs
            n3 = heating interval   (self.heat_interval, 0–30)  ×10 µs
        """
        cmd = bytes([0x1B, 0x37, self.max_dots, self.heat_time, self.heat_interval])
        printer._raw(cmd)
        logger.debug(
            f"[ESC/POS] ESC 7 → max_dots={self.max_dots}  "
            f"heat_time={self.heat_time}  heat_interval={self.heat_interval}"
        )
